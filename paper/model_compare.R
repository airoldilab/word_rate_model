...
for(ntopics in ntopics.vec){
  ...
  dwr.exc.list[[ntopics]] <- log(wrm.out$ave.param.list$phi.mat)
...
dwr.exc.ent <- lapply(dwr.exc.list,topic.dist.entropy)
lda.exc.ent <- lapply(lda.exc.list,topic.dist.entropy)
...
dwr.kl.mat <- lapply(dwr.frex.list,comp.dist.all,fun.compare=kl.dist)
lda.kl.mat <- lapply(lda.rate.list,comp.dist.all,fun.compare=kl.dist)
dwr.hel.mat <- lapply(dwr.frex.list,comp.dist.all,fun.compare=hel.dist)
